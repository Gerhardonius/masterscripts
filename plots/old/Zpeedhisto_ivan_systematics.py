...
from helpers import getHisto, customratiostyle, ratioStyle, getErrors

#
# argparser
# 
argParser = argparse.ArgumentParser(description = "Argument parser")
argParser.add_argument('--plot_directory',     action='store',      default='Zpeedhistos_ivan__systematics_test')
...
#drawPlots(plots, plotdirname, 'observed', [[(3,2),(1,0)],[(1,0),(1,0)]] , '(\mu / e)_{D} / ( \mu / e)_{E}' ,ratiostyles_observed)

graph = ROOT.TGraphAsymmErrors( histos[0][0] ) # copy histo
#graph = ROOT.TGraph( histos[0][0] ) # copy histo
for i in range(histos[0][0].GetNbinsX()):
    center    = histos[0][0].GetBinCenter(i)
    errUp   =  i
    errDown =  0 
    graph.SetPoint( i, center, 10 ) # point number, centervalue and counts
    graph.SetPointError( i, 0, 0, errDown, errUp)

plots.append( Plot.fromHisto( 'observed' , histos , texX = 'M_{ll} (GeV)', texY = 'Number of Events') )

#getErrors( plots[0].histos[0] ) 

plot_directory_ = os.path.join( plotdir, plotdirname, 'observed', 'log')
plotting.draw(plots[0],
	    plot_directory = plot_directory_,
    	    #ratio = {'histos':[(1,0),(3,2)], 'logY':False, 'style':None, 'texY': 'mm / ee', 'yRange': (0.2, 0.8), 'drawObjects':[]},
	    ratio =  None,
	    ...
	    scaling = {},
	    #legend =  ( (0.17,0.9-0.05*sum(map(len, plot.histos))/2,1.,0.9), 2),
	    #drawObjects = [drawObjects( ), graph])
	    #drawObjects = [ graph ])
            #getErrors(histo,flavor,errortype,LineColor = ROOT.kRed, LineWidth=1, style=None ):
	    drawObjects = drawObjects() + [getErrors( plots[0].histos[0][0], 'e', 'stat', LineColor=ROOT.kBlack ), getErrors( plots[0].histos[0][0], 'e', 'both', LineColor=ROOT.kBlue ) ])
