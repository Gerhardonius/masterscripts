...
# UNG, allow for module import
script_dir = os.path.dirname(__file__)

analysis_name = 'ATLAS_13TeV'
lumi = 139.           # in fb^1-

ee_data = np.loadtxt(os.path.join(script_dir,analysis_name+'/ee_data.dat'),delimiter='\t')
mm_data = np.loadtxt(os.path.join(script_dir,analysis_name+'/mm_data.dat'),delimiter='\t')
...
ee_resolution_data = np.loadtxt(os.path.join(script_dir,analysis_name+'/ee_resolutions.dat'))
mm_resolution_data = np.loadtxt(os.path.join(script_dir,analysis_name+'/mm_resolutions.dat'))
...
def calculate_chi2_noweight(ee_signal, mm_signal, signal_range = [0,5000]):
  ...
  # fits my retrieval of getSM counts
  #print 'inside ZPEEDmod_calculate chi noweights'
  #for i in range(i_low, i_high +1):
  #      print 'ee_observed', ee_observed[i]
  #for i in range(i_low, i_high +1):
  #      print 'ee_expected', ee_expected[i]
  #for i in ee_bincounts:
  #	print 'ee_bincounts', i
  #for i in range(i_low, i_high +1):
  #      print 'mm_observed', mm_observed[i]
  #for i in range(i_low, i_high +1):
  #      print 'mm_expected', mm_expected[i]
  #for i in mm_bincounts:
  #	print 'mm_bincounts', i

  # Define chi2 functions
  def chi2(mu):
    chi2_ee = np.sum([likelihood(ee_expected[i] + mu * ee_bincounts[i - i_low], ee_observed[i]) for i in range(i_low, i_high + 1)]) 

    chi2_mm = np.sum([likelihood(mm_expected[i] + mu * mm_bincounts[i - i_low], mm_observed[i]) for i in range(i_low, i_high + 1)]) 

    return chi2_ee + chi2_mm

  def chi2_Asimov(mu):
        
    chi2_ee = np.sum([likelihood(ee_expected[i] + mu * ee_bincounts[i - i_low], ee_expected[i]) for i in range(i_low, i_high + 1)]) 

    chi2_mm = np.sum([likelihood(mm_expected[i] + mu * mm_bincounts[i - i_low], mm_expected[i]) for i in range(i_low, i_high + 1)]) 

    return chi2_ee + chi2_mm

  return chi2, chi2_Asimov

def calculate_chi2_myweight(ee_signal, mm_signal, signal_range = [0,5000]):
  ...
  if len(ee_bincounts) == 1:
	if ee_bincounts[0] != 0:
  	  ee_weight[0] = integrate.quad(ee_integrand, ee_upward_fluctuation(signal_range[0]), ee_downward_fluctuation(signal_range[1]), args=(signal_range[0], signal_range[1]), epsabs=1e-30, full_output = 1)[0] / ee_bincounts[0]
  	  ee_weight[0] = min(np.abs(ee_weight[0]),1)
  	else :
  	  ee_weight[0] = 0

  	if mm_bincounts[0] != 0:
  	  mm_weight[0] = integrate.quad(mm_integrand, mm_upward_fluctuation(signal_range[0]), mm_downward_fluctuation(signal_range[1]), args=(signal_range[0], signal_range[1]), epsabs=1e-30, full_output = 1)[0] / mm_bincounts[0]
  	  mm_weight[0] = min(np.abs(mm_weight[0]),1)
  	else:
  	  mm_weight[0] = 0
